...
public class XLSXLoader {
	...
	public XLSXFile load(File xlsxFile) throws XLSXLoaderException {
		...
		try {
			return load(xlsxFile.getName(),
					new BufferedInputStream(Files.newInputStream(Paths.get(xlsxFile.getAbsolutePath()))));
		} catch (IOException e) {
	...
	/**
	 * Loads a xlsx file from an input stream. After the loading is finished,
	 * the stream is close.
	 * 
	 * @param xlsxFileStream
	 *            the stream
	 * @return an XLSXFile object
	 * @throws XLSXLoaderException
	...
	private void loadRow(final Map<Integer, XLSXColumn> columns, final List<XLSXRow> rows,
	...
	private XLSXCell createCell(Map<Integer, XLSXColumn> columns, Cell cell, String stringCellValue)
			throws XLSXLoaderException {
		if (!columns.get(cell.getColumnIndex()).getType().equals(XLSXColumnType.TEXT)) {
			throw new XLSXLoaderException(String.format("%s cell has a different type %s than %s",
					"[" + cell.getRowIndex() + " " + cell.getColumnIndex() + "]", "TEXT",
					columns.get(cell.getColumnIndex()).getType()));
		}
	...
	private XLSXCell createCell(Map<Integer, XLSXColumn> columns, Cell cell, Double numericCellValue)
			throws XLSXLoaderException {
		if (!columns.get(cell.getColumnIndex()).getType().equals(XLSXColumnType.NUMERIC)) {
			throw new XLSXLoaderException(String.format("%s cell has a different type %s than %s",
					"[" + cell.getRowIndex() + " " + cell.getColumnIndex() + "]", "NUMERIC",
					columns.get(cell.getColumnIndex()).getType()));
		}
		XLSXCell xlsxCell = new XLSXCell();
		xlsxCell.setColumn(columns.get(cell.getColumnIndex()));
	...
	private XLSXCell createCell(Map<Integer, XLSXColumn> columns, Cell cell, Boolean booleanCellValue)
			throws XLSXLoaderException {
		if (!columns.get(cell.getColumnIndex()).getType().equals(XLSXColumnType.BOOLEAN)) {
			throw new XLSXLoaderException(String.format("%s cell has a different type %s than %s",
					"[" + cell.getRowIndex() + " " + cell.getColumnIndex() + "]", "BOOLEAN",
					columns.get(cell.getColumnIndex()).getType()));
		}
	...
	private XLSXCell createCell(Map<Integer, XLSXColumn> columns, Cell cell, Date dateCellValue)
			throws XLSXLoaderException {
		if (!columns.get(cell.getColumnIndex()).getType().equals(XLSXColumnType.DATE)) {
			throw new XLSXLoaderException(String.format("%s cell has a different type %s than %s",
					"[" + cell.getRowIndex() + " " + cell.getColumnIndex() + "]", "DATE",
					columns.get(cell.getColumnIndex()).getType()));
		}
