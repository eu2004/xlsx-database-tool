...
public class XLSXFileTableExporter {
	...
	public Path exportXLSXFileTableToSQLScript(String filePath) throws FileNotFoundException, UnsupportedEncodingException {
		...
		try(PrintWriter writer = new PrintWriter(Paths.get(filePath).toFile(), "UTF-8")){
	        ...
	        for(XLSXRow row : rows) {
	        	writer.println(SQLQueriesUtils.generateSQLInsertRow(xlsxFileTable.getTableName(), xlsxFileTable.getColumns(), row));
	        }
		};
